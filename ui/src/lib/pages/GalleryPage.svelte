    ...
    <div class="body">
      ...
      {#if metadata}
        <div class="meta">
          ...
          {:else}
            <div class="muted">no embedded workflow - enable embed_metadata in the step's result</div>
          {/if}
        </div>
...
<style>
  ...
  .cell {
    ...
    gap: 0.3rem; color: var(--muted); font-weight: 500; font-size: 0.75rem;
  }
  .cell:hover, .cell.active { border-color: var(--accent); filter: none; }
  .cell img, .cell video {
    width: 100%; aspect-ratio: 1; object-fit: cover; border-radius: 5px; display: block;
  }
  .audio { aspect-ratio: 1; display: grid; place-items: center; }
  .caption { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .detail { position: sticky; bottom: 1rem; margin-top: 1rem; box-shadow: 0 6px 24px rgb(0 0 0 / 0.35); }
  ...
  .icon { display: inline-flex; padding: 0.3rem 0.45rem; }
  .body { display: flex; gap: 1rem; align-items: flex-start; flex-wrap: wrap; }
  .body img, .body video { max-width: min(480px, 100%); border-radius: 6px; }
  .meta { display: flex; flex-direction: column; gap: 0.2rem; font-size: 0.85rem; }
  .meta .muted { margin-right: 0.4rem; }
</style>
