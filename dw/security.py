...
def validate_path(path: Union[str, Path], base_dir: Optional[str] = None, allow_create: bool = True) -> str:
    """
    ...
    for pattern in DANGEROUS_PATTERNS:
        if re.search(pattern, normalized_path, re.IGNORECASE):
            raise PathTraversalError(f"Path contains dangerous pattern matching {pattern}")
    ...
    if base_dir:
        try:
            ...
            try:
                ...
                raise PathTraversalError(f"Path on different drive than allowed directory: {resolved_path}")
        except (OSError, ValueError) as e:
    ...
    if len(filename) > MAX_FILENAME_LENGTH:
        raise InvalidInputError(f"Filename too long: {len(filename)} > {MAX_FILENAME_LENGTH}")
...
def validate_string_input(value: str, max_length: int = 1000, allow_empty: bool = False) -> str:
    """
...
def safe_join_path(*parts: str) -> str:
    ...
    for part in parts:
        ...
        if '..' in part or '/' in part or '\\' in part:
            raise InvalidInputError(f"Path component contains invalid characters: {part}")
