    ...
    with patch.object(Pipeline, '__init__', mock_pipeline_init):
        with patch.object(Pipeline, 'load', mock_pipeline_load):
            with patch.object(Pipeline, 'run', mock_pipeline_run):
                ...
                logger.info(f"   Steps passed: {captured_arguments[-1]['num_inference_steps']}")
                ...
                workflow_def["steps"][0]["pipeline"]["arguments"]["num_inference_steps"] = 30
                ...
                assert len(captured_arguments) == 2, f"Expected 2 runs, got {len(captured_arguments)}"
                logger.info(f"✅ Both runs executed")
                
                run1_prompt = captured_arguments[0].get("prompt")
                run2_prompt = captured_arguments[1].get("prompt")
                
                assert run1_prompt == "a cat", f"Run 1 should have 'a cat', got '{run1_prompt}'"
                logger.info(f"✅ Run 1 used correct prompt: '{run1_prompt}'")
                
                assert run2_prompt == "a dog", f"Run 2 should have 'a dog', got '{run2_prompt}'"
                logger.info(f"✅ Run 2 used NEW prompt: '{run2_prompt}'")
                
                assert run1_prompt != run2_prompt, "Arguments should be different between runs!"
