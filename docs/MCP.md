...
| `get_job(job_id)` | `job_id` | Get a job's status, warnings, output manifest, error and traceback; each manifest entry's `subfolder` says whether the step declared its output `final`, `intermediate`, or nothing. A running job also carries `progress` (below) |
