...
def test_pipeline_caching():
    ...
    def mock_pipeline_load(self, *args, **kwargs):
        ...
        loaded_models[self.pipeline_definition.get("from_pretrained_arguments", {}).get("model_name", "unknown")] = self.pipeline
    ...
    with patch.object(Pipeline, '__init__', mock_pipeline_init):
        with patch.object(Pipeline, 'load', mock_pipeline_load):
            ...
            logger.info(f"✅ First run: Pipeline loaded (load_call_count={first_load_count})")
            ...
            assert first_load_count == 1, f"Expected 1 load on first run, got {first_load_count}"
            logger.info(f"✅ First run loaded exactly once")
            
            assert second_load_count == 1, f"Expected no additional loads on second run, got {second_load_count}"
            logger.info(f"✅ Second run reused cached pipeline (no reload)")
            
            # Note: We now create a new wrapper but reuse the underlying model
            assert action1.pipeline is action2.pipeline, "Expected same underlying pipeline model to be reused"
            logger.info(f"✅ Both runs reused the same underlying model (pipeline.pipeline)")
...
def test_pipeline_caching_different_steps():
    ...
    with patch.object(Pipeline, '__init__', mock_pipeline_init):
        with patch.object(Pipeline, 'load', mock_pipeline_load):
            ...
            assert load_call_count == 2, f"Expected 2 loads (one per step), got {load_call_count}"
            assert action1.pipeline is action1_reuse.pipeline, "Step1 underlying model should be reused from cache"
            assert action1.pipeline is not action2.pipeline, "Step1 and step2 should have different underlying models"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("This significantly improves performance by avoiding repeated model loading.")
